...
export type TeacherEmployeeIdSettings = Pick<
  TeachersSettings,
  'idPrefix' | 'idTemplate' | 'idDigits' | 'idStartSeq' | 'idRestartAnnually'
>;

/** Format employee ID using tokenized template ({PREFIX}, {YYYY}, {YY}, {MM}, {SEQ}). */
export function formatTeacherEmployeeId(
  seq: number,
  settings: Partial<TeacherEmployeeIdSettings> = {},
  dateInput?: Date | string,
): string {
  const prefix = settings.idPrefix?.trim() || DEFAULT_TEACHERS_SETTINGS.idPrefix;
  let template = settings.idTemplate?.trim() || DEFAULT_TEACHERS_SETTINGS.idTemplate || '{PREFIX}-{SEQ}';
  const digits = Math.max(1, Math.min(8, Number(settings.idDigits) || 4));
