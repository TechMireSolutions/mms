...
**Rules (norms SSOT):** `mms-data-layer.mdc` §6 · `mms-module-architecture.mdc` §6–§7 · `mms-core.mdc` · `mms-auth-security.mdc` · `mms-form-architecture.mdc` · `mms-performance.mdc`. DDL migrations → `mms-schema-migrate`. Backend API → `mms-backend-api`. Work UI → `mms-module-work`. Outbox/Audit → `mms-audit-trail`. Background Jobs → `mms-background-jobs`.
...
- **Batched Single-Statement Updates**: Implementations **must** use a single batched SQL statement (`inArray(table.id, ids)`) — per-row iteration loops ($N+1$ query pattern) are strictly banned (`mms-performance.mdc` §1).
