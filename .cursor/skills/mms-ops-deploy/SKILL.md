...
**Rule (norms SSOT):** `mms-ops-infrastructure.mdc` · `mms-auth-security.mdc` · `mms-completion-review.mdc`.
...
`mms-ops-infrastructure.mdc`, `mms-auth-security.mdc`, `mms-data-layer.mdc` (Cursor mirrors use `.mdc` — sync docs only; agent canon is `.md`)
