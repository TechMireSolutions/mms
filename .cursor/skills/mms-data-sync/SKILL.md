...
**Legacy / hybrid only.** New REST entity work → skill **`mms-query-factories`** (+ `mms-data-layer.mdc`). Backup wipe-restore UI → **`mms-backup-restore`**. Do not expand `useLiveCollection` for REST-migrated entities.
