...
- **Explicit Column Projections (`check:db-projections`)**: Bare `.select().from(table)` and `SELECT *` are strictly banned (`mms-performance.mdc`). Every query must supply an explicit typed column projection (`db.select({ id: table.id, name: table.name }).from(table)`).
