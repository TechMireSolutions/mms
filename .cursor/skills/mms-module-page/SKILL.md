...
| §2 | Three-tier shell norms | `mms-module-architecture.mdc` · chrome (`PageHeader` / `ResponsiveAccordionTabs`) → `mms-ui-ux-design.mdc` |
...
| §6 | Soft-delete Work UX + RBAC omit | skill **`mms-soft-delete`** · `mms-module-architecture.mdc` (+ sessions/RBAC middleware → `mms-auth-security.mdc`) |
