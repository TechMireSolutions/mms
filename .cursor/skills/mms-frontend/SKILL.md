...
`mms-completion-review.mdc` — typecheck → FE lint → tests if hooks touched. Layout: spot-check 375 / 768 / 1440. Prefer Query `signal`; no new `useMemo`/`useCallback` by default (React 19: prefer `useEffectEvent`, `startTransition`, `useDeferredValue`).
