...
**Rules (norms SSOT):** `mms-hooks.mdc` · `mms-data-layer.mdc` §3 · `mms-api-interface.mdc` · `mms-dry.mdc` · `mms-performance.mdc` §5.
...
4. Colocate TanStack Query v5 `queryOptions` / `mutationOptions` with those keys to deduplicate inflight network requests and cache query states. Strict ban on manual `useEffect(() => { fetch(...) }, [])` calls for server state (`mms-performance.mdc`).
