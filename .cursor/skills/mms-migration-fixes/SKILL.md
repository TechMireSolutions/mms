...
| Custom tabs & fields (DFS) typed + FORCE RLS | `custom_tabs` & `custom_fields` DB tables (`workspaceSubdomain: text` + composite PK + app-generated `text` IDs + `FORCE RLS` + tenant-scoping policy; DFS migration `migrations_drizzle/0030_custom_fields.sql` + journal `idx:30`) via Fastify 5 `/api/v2/modules/:module/tabs` (`dynamicFormPlugin`, `DFS.md` v2.0); dynamic Zod validation via `buildDynamicValidationSchema` (decimal-as-string, E.164); server-side `customData` re-validation on entity save (`DFS.md` §4.5); audit via `auditPreHandler`/`auditOnSend` → existing `audit_logs` table (`onSend` returns `payload`, re-fetches new state); RBAC `authenticateTenant` + `can(module, 'editSetup')`; `/check-unique` probe not audited |
