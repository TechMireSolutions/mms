...
5. Claim next job with **`FOR UPDATE SKIP LOCKED`** (`worker.ts`) — norms → `mms-module-architecture.md` §5.
...
- Depend on the in-process runner for critical multi-instance production work without adding a durable queue — `mms-module-architecture.md` §5.
