...
**Fix:** Do not regress Messaging clear or QB papers/results variants without an explicit product change. Users real soft-delete is shipped (`0027_tenant_users_soft_delete`). Push SQL pagination when touching list hot paths (`mms-data-layer.md`).
...
**Fix:** Prefer typed tables + FORCE RLS for new shareable/secret data; `pgTable` + migration for custom tabs per `mms-fields.md`.
