...
- **Bulk Actions**: Use shared `BulkSelectionBar` (`@/components/ui/BulkSelectionBar`; `floating` \| `inline`) — do not invent parallel selection chrome. Bars appear only when records are selected. Enforce backend permission checks, calculate partial failures, and audit all bulk updates/deletions.
...
| **Work UX** | `ErrorState` + retry **and** hint description (`loadFailedHint` or module equivalent); Cmd/Ctrl+N opens create when `canWrite` and not in trash; Filters menu SSOT (no duplicate preset chip bar); selection UI via `BulkSelectionBar`; single resolved directory `viewMode` (`table` \| `cards`; cards `< md`, table `md+`); same server pagination for both views; column visibility **and** width via `useModuleColumnLayout` (local + `/column-preferences`, merge preserves device widths). |
