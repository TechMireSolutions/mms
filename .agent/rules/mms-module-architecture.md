...
- **Scaling**: The worker is a separate process (docker-compose `worker` service; PM2 `mmsv2-worker`) and job state lives in Redis — horizontal scaling is safe. Never move job execution back into the API process. Workflow → skill **`mms-background-jobs`**.
