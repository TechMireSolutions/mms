...
| **Soft-delete** | `DELETE` + `POST :id/restore` (+ bulk when applicable); Work `showDeleted` / `includeDeleted` trash UI; detail-drawer archive chrome + Restore; hide messaging actions when deleted; manifest `softDelete` metadata. Schema strip → `mms-data-layer.md`. |
