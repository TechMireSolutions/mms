...
|---|---|
| **Pool Sizing & Timeouts** | Size pool via `PG_POOL_MAX` (default 20). `withTenant` applies `PG_STATEMENT_TIMEOUT_MS` (30s) and `PG_IDLE_IN_TX_TIMEOUT_MS` (15s), with support for tighter route-level budgets on hot paths via `statementTimeoutMs` (e.g. 5000ms). |
...
| **Tenant Boundary API** | Three entry points, all in `apps/backend/src/db/tenant-context.ts`: `withTenant(tenant, cb)` (default, read-write on the primary), `withTenantRead(tenant, cb)` (read replica + Postgres `read only` access mode — use for list/count/report paths), and `withGlobalTenant(cb)` (RLS intentionally bypassed: platform admin, migrations, worker, seeding, whole-DB backup). **`withTenant` fails closed** — a falsy tenant throws unless `{ allowGlobal: true }` is passed, because an empty tenant sets `app.rls_bypass = 'on'` and makes every RLS policy match ALL rows. |
| **Read-Your-Writes** | `withTenantRead` may hit a replica, so its result can lag a write that happened moments earlier. Never use it for a read that must observe a write from the same request or an immediately preceding one (create-then-refetch, optimistic-update verification, uniqueness checks) — use `withTenant` there. When `READ_REPLICA_DATABASE_URL` is unset or equal to `DATABASE_URL`, both go to the primary and this is moot; `mms_db_pool_connections{pool="replica"}` appears in `/metrics` only when reads are actually split. |
| **Tenant-Scoped ID Lookups** | An id-only lookup is **never** implicitly tenant-safe. Every repository lookup that takes an id must also take the workspace as its first argument and apply `eq(table.workspaceSubdomain, tenant)` in SQL — RLS is the backstop, not the isolation mechanism. Add an explicit `…ByIdGlobal` variant only when platform code genuinely needs cross-workspace access, and never call it from a tenant route. |
| **New Tenant Tables** | `FORCE ROW LEVEL SECURITY` + RLS policy (`tenant_id = current_setting('app.current_tenant', true)::uuid` or subdomain match). Writes route through `withTenant`. |
