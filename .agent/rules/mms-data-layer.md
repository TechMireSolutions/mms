...
| **Pool Sizing & Partitioning** | Size pool via `PG_POOL_MAX` (default 20). Dedicated connection pool partitioning: worker subprocesses run on a partitioned pool (`role: 'worker', max: 10`) isolated from user-facing API pools (`role: 'http', max: 20–30`). `withTenant` applies `PG_STATEMENT_TIMEOUT_MS` (30s) and `PG_IDLE_IN_TX_TIMEOUT_MS` (15s). Route-level statement timeout tiers: interactive reads `3000ms`, standard mutations `5000ms`, background jobs/sync up to `60000ms`. |
...
| **Prepared Statements & Aggregations** | Hot point lookups by ID compile Drizzle prepared statements with placeholders (`sql.placeholder`) to eliminate SQL AST generation and query re-planning overhead. Child collections hydrate via consolidated single-statement SQL aggregations (`json_agg` / `jsonb_agg`) as the default canonical path (`MMS_DISABLE_AGGREGATED_CHILD_HYDRATION !== 'true'`), slashing 12 parallel child queries into 1 single roundtrip. |
| **Server Caching Architecture** | Multi-Tier Caching Architecture (`multiTierCache.ts`): L1 in-process LRU (`lru-cache`, max 5,000 items, TTL 5m, 50 MB hard heap ceiling `L1_MAX_HEAP_BYTES`, `sizeCalculation`) + L2 Redis fallback with single-flight Promise coalescing. Setup lookups and singleton configs are wired to `getOrSetMultiTier`. Deterministic cross-node invalidation via Redis pub/sub channel `mms:cache-invalidation`. TTL 60s for metrics/KPIs, 300s for lookups/config/branding. Mutating ops must evict keys and broadcast via `/api/ws` and Redis pub/sub. |
| **Worker Queue Architecture** | BullMQ 6 (`bullmq: ^6.3.4`) backed by Redis 6 (`ioredis: ^6.0.0`). CPU-intensive/heavy tasks (PDF rendering via Typst, bulk export) run in sandboxed child processes (`useWorkerThreads: false`) with `--max-old-space-size=512` and a hard 60s execution timeout (`JOB_EXECUTION_TIMEOUT_MS`). Dynamic tenant fair-share priority scheduling (Bands 1–3) based on active inflight jobs in Redis. |
...
- **REST Entities Ban:** Contacts, Students, Faculty, Sessions, Users, Enrollments, Finance, Accounting use Query/REST — never `getCollection` or `saveCollection`.
...
1. **Architectural Deletion Taxonomy:**
   - **Bucket 1 (Soft-Delete):** Primary business entities (`contacts`, `students`, `faculty`, `sessions`, `enrollments`, `finance_invoices`, `accounting_accounts`). User self-service restore, historical reporting continuity.
