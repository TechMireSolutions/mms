...
| `platform_users` | Apex platform super-users — **not** tenant-scoped; unique `email` |
| `tenant_users` | Madrasa auth users — scoped by `workspace_subdomain`; unique `(workspace_subdomain, login_email)` |

Auth credentials for platform and tenant operators live in **`platform_users`** / **`tenant_users`**, not in JSON blobs. Legacy `platform_super_users` object and tenant `users` collections are migrated on startup (`012_migrate_users_to_tables.ts`); tenant writes dual-sync to the `users` collection for the Users module UI until full REST migration.
