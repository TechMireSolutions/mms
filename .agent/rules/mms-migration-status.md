...
| Automated tests | Shared + backend; frontend hooks; Playwright onboard + Contacts/Students/Attendance + teacher/invoice/payment + session/class/enrollment + messaging template/campaign + accounting accounts/journal + users Add User (inactive teacher) + module tier shells + settings | Broaden remaining write flows (obligations, etc.) — `mms-testing-observability.md` |
...
- **Onboarding E2E critical path**: `e2e/tests/onboarding-login.spec.ts` covers platform setup → tenant onboard → contacts (with phone/email) → student with guardians → attendance bulk submit → module tier shells → teacher create → finance invoice → record payment → session + class → enrollment wizard → messaging preset template → SMS campaign compose + message log → accounting chart accounts + balanced journal entry → Users Add User (contact-linked inactive Teacher).
- **Users list name/email after REST sync**: `useCollectionSync` caches REST payloads with `saveCollectionCacheOnly` (no `/api/db` echo wipe); `normalizeWorkspaceUser` falls back to `loginEmail`; contact hydrate prefers non-empty composed names.
