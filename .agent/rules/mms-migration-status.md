...
| Other-module document-store prefs | Contacts + Students Setup SSOT closed (`custom_tabs`, typed `*_field_configs` / `*_module_preferences` / `*_user_column_prefs` + REST). Residual: Teachers and other modules’ `*_field_config` / prefs / column prefs still on `objects` → typed REST — `mms-fields.md` / `mms-data-layer.md` |
...
| Work REST parity (Teachers/Users/Sessions) | Students Work closed: SQL page/filter, table\|cards, trash/drawer archive chrome, metrics, server CSV (`POST /api/students/export/csv` + audit + `students:export`). Residual: Teachers / Users / Sessions list/unpaged dumps, drawer gaps, Teachers server export → Contacts parity — `mms-module-architecture.md` §3/§7 |
