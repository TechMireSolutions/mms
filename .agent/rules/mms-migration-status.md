...
| **Live Push & Aggregates** | Closed for primary modules (Contacts/Students/Faculty/Sessions/Enrollments). Residual: secondary module WS emit/subscribe and comparison mode dumps. | WS `/api/ws` invalidate + SQL `GROUP BY` aggregates (`mms-core.md`, `mms-reports.md`). |
...
| **Sessions** | Storing JWTs in `localStorage`; skipping platform `/me` session probe on boot; failing to invalidate active sessions and tokens when a user/faculty member is soft-deleted; allowing soft-deleted accounts to authenticate. | `mms-auth-security.md` |
...
| **Person Modules** | Persisting contact profile fields on `students.custom_data` or `faculty.custom_data` when `contactId` is set; filtering JSONB instead of joining `contacts`. | `mms-data-layer.md`, `mms-form-architecture.md` |
