...
| Script files | `kebab-case.sh` | `scripts/deploy-on-server.sh`, `scripts/check-migration-indexes.mjs` |
| Core module imports | `node:` prefix | `node:fs/promises`, `node:crypto`, `node:path`, `node:async_hooks` |

### Core Protocol Imports & WHATWG URLs
- **Node Protocol Imports**: Always prefix core module imports with `node:` (`node:fs`, `node:path`, `node:crypto`, `node:async_hooks`). Unprefixed imports (`import fs from 'fs'`) are forbidden to prevent namespace collisions and optimize resolution.
