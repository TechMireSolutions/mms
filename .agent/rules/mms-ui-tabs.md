...
## PageHeader — module command centre

`PageHeader` sits **above** `ResponsiveAccordionTabs` and stays visible on all tiers (`mms-module-architecture.md` §2).

```tsx
<PageHeader
  title={t('nav.contacts')}
  subtitle={t('page.contacts.subtitle')}
  actions={<><ExportBtn /><DuplicatesBtn /><ActionButton onClick={onAdd}>…</ActionButton></>}
/>
<ContactsCommandMetrics total={total} shown={filteredCount} />
```

| Element | Rule |
|---------|------|
| Title | `t('nav.*')` or module page key |
| Subtitle / metrics | Permission-scoped counts (total, filtered, pending) — not foreign-module stats |
| Primary create | Always in `actions` — not gated on `activeTab` |
| Export / integrity | Module-scoped tools in `actions` (e.g. export, duplicates) |
| Tier-only actions | Inside tier content (toolbar, report filters) |

Reference: `Contacts.tsx`.
