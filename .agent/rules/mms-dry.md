...
- **SSOT Entity UI Descriptors**: Before creating ad-hoc table column definitions, card metadata arrays, filter chips, or detail drawer layouts for domain entities, consume declarative entity descriptors via `createEntityDescriptor` (`@/components/common/entityRegistry`) to keep table columns, card tiles, filter chips, and drawer fields unified.
