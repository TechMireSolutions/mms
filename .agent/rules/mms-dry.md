...
- **FE Work chrome**: Before adding selection bars, warning/archive banners, or quick-action buttons, reuse `BulkSelectionBar` / `WarningCallout` / `QuickActionButton` / `formStyles` surface tokens (`WORK_SURFACE`, `WORK_SURFACE_INNER`, `DETAIL_SECTION_TITLE`) under `apps/frontend/src/components/ui/` — do not copy markup across modules.
