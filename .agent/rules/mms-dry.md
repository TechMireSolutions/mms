...
- **FE Work chrome**: Before adding selection bars, empties, KPI strips, warning/archive banners, or quick-action buttons, reuse `BulkSelectionBar` + `BulkSelectionActions` / `EmptyState` / `ModuleCommandMetricsGrid` / `WarningCallout` / `QuickActionButton` / `formStyles` surface tokens (`WORK_SURFACE`, `WORK_SURFACE_INNER`, `DETAIL_SECTION_TITLE`) under `apps/frontend/src/components/ui/` — do not copy markup across modules. Column gates: pass `isColumnVisible` into leaves — do not fan parallel `show*` / `visibleColumns` boolean objects.
