...
5. **Frontend** — Query hooks + `useModulePermissions(manifest)` (`mms-frontend`, `mms-data-layer.md`)
...
All backend API routes, repository queries, validation, and schema definitions for dynamic forms must follow **`mms-form-architecture.md`** (or the corresponding Cursor rule `mms-form-architecture.md`). Specifically:
...
Tenant writes must use `withTenantTransaction` / SET LOCAL RLS (`mms-data-layer.md`), including `app.current_user_id` for audit triggers. New tables: composite tenant PK + FORCE RLS. Bulk PUT upsert only — never `replaceForWorkspace` wipe. OAuth/API secrets: FORCE-RLS credential tables — not `objects` KV. Messaging REST: `routes/tenant/messaging.ts`.
