...
**Rules (norms SSOT):** `mms-data-layer.md` §5–§6 · `mms-auth-security.md` §5 · `mms-performance.md` §1 · `mms-ops-infrastructure.md` · `mms-structure-naming.md` · `mms-api-interface.md` · `mms-form-architecture.md`. Audit trail workflow → **`mms-audit-trail`**. Soft-delete workflow → **`mms-soft-delete`**.
...
Migration applies cleanly on empty + existing DB; Zod and Drizzle types compile with `pnpm typecheck`; `bash scripts/check-migrations.sh` is green — `mms-completion-review.md`.
