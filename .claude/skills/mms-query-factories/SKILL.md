...
2. Reuse the shared `lib/query/` factories first — `createModuleCrudMutations` / `createModuleQueryInvalidator` / `createModuleSetupConfigApi` / `createPersonModuleResolveQueries` — before hand-rolling per-module factories (`mms-hooks.md`). Thin module facades under `@/tenant/hooks/collections/{module}` or `@/platform/hooks/collections/{module}` wrap them.
