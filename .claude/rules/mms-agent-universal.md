...
- **Validation:** Zero-trust DTOs via `@mms/shared` Zod + backend `parseRequest` — `mms-api-interface.md`, `mms-core.md`.
- **State:** Prefer unidirectional flow; pure helpers for transforms — Query policy `mms-data-layer.md` §4.
- **Concurrency & Cancellation:** Pass `AbortSignal` into `apiFetch` / Query `queryFn`; combine signals via `AbortSignal.any()`; clear timers/observers — `mms-data-layer.md`, `mms-api-interface.md`.
- **Resilience:** Error boundaries on heavy sections and lazy routes — `mms-testing-observability.md` §4.
...
- **TypeScript:** Strict mode. Use `unknown` + narrowing — never `any`. Double assertions (`as unknown as T`) to bypass validation are forbidden; validate via Zod or type predicates (`is`). Prefer `import type` and `satisfies`. Zero non-erasable syntax (`enum`, `namespace`, constructor parameter properties) — details `mms-dependencies.md`.
...
- **Git:** Conventional Commits (`feat`/`fix`/`chore`). **NEVER run `git add`, `git commit`, or `git push` (or any equivalent) unless the user explicitly says "commit" in that exact message. Never push to GitHub under any circumstance — the user handles all pushes themselves. Do not stage or commit as a "convenience" after edits. The user owns all git operations.**
...
- **Enforcement principle:** A norm is either **machine-enforced** (lint rule, ratchet script, CI check, hook, test) or explicitly labelled **advisory**. When a rule is added, land its check in the same change or say why it cannot have one; prefer a code-level fix over a new paragraph of prose.
- **Tool neutrality:** Rule bodies are mirrored into three tools. Describe outcomes and commands, never one client's internal tool names or parameter syntax — tool-specific mechanics belong in that tool's frontmatter.

## Anti-Patterns & Banned Operations

- ❌ **NEVER bypass validation with type casting:** no `as unknown as T` or `any` to force type agreement.
...
- ❌ **NEVER restate an owned norm:** when another rule owns the topic, link to it instead of copying it — `rules/README.md` lists the owner per topic.
