...
- **Git:** Conventional Commits (`feat`/`fix`/`chore`). **NEVER run `git add`, `git commit`, or `git push` (or any equivalent) unless the user explicitly asks in that message. Do not stage or commit as a "convenience" after edits. The user owns all git operations.**
