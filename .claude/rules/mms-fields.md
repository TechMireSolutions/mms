...
| **Custom fields** | Tenant `custom_data` JSONB + field config registries | Constrained via field registries — not free-form layout engines |

- Free-form dynamic form compilers / visual schema generators are **banned** — forms are strictly registry- and shared-schema-driven.
...
1. **Shared** — type + Zod in `@mms/shared` (`FIELD_TYPES_META`, `createFormCustomFieldHelpers`, field config schemas)
