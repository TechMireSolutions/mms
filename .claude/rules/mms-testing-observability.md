...
`pnpm test` executes tests across the monorepo workspaces:
- **`@mms/shared`**: Unit tests for validation schemas, pure utilities, date/currency formatters, and permission calculations using `node:test` + `node:assert/strict` or Vitest.
- **`mms-backend`**: Integration tests utilizing Fastify's `inject()` and `node:test` / Vitest. The Node 24 native test runner (`node:test`) automatically awaits subtests without manual promise management. Cover tenant auth/RBAC/token rotation **and** apex platform auth/settings/workspaces allow+deny (wrong host → `403`, cookie session, `super_user` / permission gates, reset-database validation).
- **Lightweight Scripts & CLI Testing**: Use `--experimental-strip-types` to execute `.ts` test scripts directly without upfront build steps.
...
### When to Write Tests
1. **Shared Package**: All new non-trivial pure function exports in `@mms/shared` must include unit tests (`node:test` / Vitest).
