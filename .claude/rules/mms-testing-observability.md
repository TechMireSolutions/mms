...
- **Deterministic Execution (Zero Skip Latches):** Banned: `if (!isDbAvailable) return;`. Mocks use in-memory repository fakes with Fastify `inject()`. Real DB integration tests must fail if database infrastructure is unavailable; never silently pass.
...
- **Coverage Mandate:** Unit tests required for all non-trivial `@mms/shared` exports, regression fixes, and auth/RBAC route rules (`inject()` allow+deny). Enforce coverage floors (FE lines 41, BE lines 45).
- **Playwright Best Practices:** Strictly ban fixed `waitForTimeout`. Use web-first assertions (`expect(locator).toBeVisible()`) and user-centric locators (`getByRole`, `getByLabel`, `getByText`). Keep responsive and axe smoke specs (`e2e/tests/a11y-shell.spec.ts`) green.

## 2. API Error Expectations
...
## 4. Frontend Resilience & Boundaries
...
## 7. Client Error Surface & Retry Policy
