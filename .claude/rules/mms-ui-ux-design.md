...
Rules governing the strictly typed, component-driven, accessible UI/UX architecture of the Madrasa Management System (MMS). These rules apply **equally to both tenant workspaces and the platform apex**; platform must not fork its own UI primitives.
...
- Extend central primitives safely when custom variations are needed. Do not implement ad-hoc primitives in feature folders (tenant or platform).
...
- **Command Metrics Grid**: Use `<ModuleCommandMetricsGrid />` (`components/ui/ModuleCommandMetricsGrid.tsx`) for command-centre **and** report KPI **StatCard strips** (staggered entrance). Do not write custom `grid-cols-*` StatCard strips for that pattern. Intentional exceptions (not metrics strips): filter nav grids, centered Card KPI tiles (e.g. ResultsViewStats), Budget/AccountingDashboard custom tiles, and the dense reports home `KPICardsGrid` (trend/subtext density differs from `StatCard`).
