...
| SQL pagination / oversized shells | Teachers / Users / Sessions / Enrollments / Finance / Accounting Work list/metrics SQL-page closed; Teachers SQL bulk status + widget aggregates + soft-delete audit (`deleted_by`/`deletion_reason`) + dedicated `teachers.delete` closed; Sessions soft-delete audit + `/report-aggregates` + server CSV export closed; Enrollments typed soft-delete audit + captureDeletionReason Work UX + server CSV export closed; Finance `listInvoicesPage`/`listPaymentsPage` SQL-page + FE `useFinanceInvoicesPaginated`/`useFinancePaymentsPaginated` hooks closed; Accounting `listAccountsPage`/`listEntriesPage`/`listFiscalYearsPage` SQL-page + FE `useAccountingAccountsPaginated`/`useAccountingEntriesPaginated`/`useAccountingFiscalYearsPaginated` hooks closed. Residual: remaining modules still in-memory page after load; a few ~220–275 shells → SQL page/filter; split behind barrels — `mms-data-layer.md` / `mms-structure-naming.md` |
