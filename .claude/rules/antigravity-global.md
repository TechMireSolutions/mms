...
- **TypeScript:** Strict mode. Use `unknown` + narrowing — never `any`. Prefer `import type` and `satisfies` operator for literal/config safety without widening. Ban non-erasable syntax (`enum`, `namespace`, constructor parameter properties) for native Node 24 type stripping. Dedicated-PR targets (`noUncheckedIndexedAccess`, `verbatimModuleSyntax`, `erasableSyntaxOnly` on TS 5.8+) → `mms-dependencies.md`.
