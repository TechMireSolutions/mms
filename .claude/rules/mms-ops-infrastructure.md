...
- **Node.js**: Match root `package.json` `engines.node` (currently **`>=24.14.0`**). Align CI/Docker images with the same major — do not leave Dockerfile on a newer Node/pnpm pin than `engines` / `packageManager`.
...
CI Node/pnpm images must match root `engines` / `packageManager`. Never commit `.env` or secrets in artifacts.
