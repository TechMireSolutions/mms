...
- **RBAC JSONB Merging**: To prevent destroying data omitted from partial frontend payloads, always write custom fields using `||` concatenation with `COALESCE`:
  ...
  ```
- **Partial PUT merges**: Merge defined patch keys onto the existing row **before** prepare/normalize so omitted collections are not wiped with `undefined` / `[]` defaults.
---
