...
- **Stack**: Fastify 5, Drizzle ORM (`pg`), Vitest, Zod (schemas), and ESM.
...
- **Health Checks**:
  - `GET /health` (liveness).
  - `GET /ready` returns `200` if PostgreSQL ping (`pingDatabase`) succeeds, else `503`.
...
- **Decoupling**: Never import `pg` or Drizzle drivers inside route handlers. Use Zod schemas in `validation/*Schemas.ts` with `parseRequest` for request body validation.
...
- **Ephemeral State**: OTP codes, handoff codes, and refresh token hashes must be persisted in PostgreSQL (`auth_artifacts`), not in-memory maps.
