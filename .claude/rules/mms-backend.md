---
description: Fastify routes, middleware, services, validation, WhatsApp, security
paths:
  - "apps/backend/src/**"
...
apps/backend/src/
  ...
  lib/                        # shared infra (tenantContext, httpErrors, syncLimits, zodRequest)
  db/
...
| `MMS_SYNC_REQUEST_TIMEOUT_MS` | No | Bulk sync processing cap (default 2 min) |
...
| `/api/contacts` | `GET /`, `GET /count`, `POST /`, `PUT /:id`, `DELETE /:id`, `GET /:id/whatsapp-status` | `validation/contactSchemas.ts` | same + `whatsAppService` side effects |
...
- Validate bodies: **Zod** `safeParse` via `lib/zodRequest.ts` (`parseRequest` + `replyValidationError`) on all write endpoints
...
| `POST /sync` | Admin only (`canBulkSync`) — **body limit** `MMS_SYNC_MAX_BODY_BYTES`; **timeout** `MMS_SYNC_REQUEST_TIMEOUT_MS` via `withSyncTimeout` |
...
| File | Covers |
|------|--------|
| `__tests__/app.health.test.ts` | `/health`, `/ready` |
| `__tests__/app.security.test.ts` | RBAC download sync, unauthenticated deny |
| `__tests__/auth.integration.test.ts` | Login subdomain, refresh rotation, 2FA gate, tenant JWT binding |
| `__tests__/services/rbacService.test.ts` | Permission matrix |
| `__tests__/services/twoFactorService.test.ts` | OTP / refresh helpers |
