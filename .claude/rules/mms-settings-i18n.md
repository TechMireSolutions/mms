...
- Always run `validateWorkspaceBackupJson` / `validateAndNormalizeSnapshot` (and subdomain match) **before** wipe-restore; treat validation-only as the dry-run — never commit a partial restore. Envelope/KDF mechanics → `mms-data-layer.md`.
- All copy via `backup.*` keys (en/ar/ur/fa). Map HTTP `408` / `backup.syncTimeout` to localized timeout messaging.
...
- **Settings-Aware `Intl` Formatting (SSOT)**: All dates/money via settings-aware `formatDate` / `formatMoney` from `@mms/shared` (or `@/lib/utils` / `@/lib/db` wrappers) + `Intl` — never raw `.toLocaleDateString()` / `.toLocaleString()` or hardcoded `PKR` / `₨` prefixes. Currency hooks: `useFinanceCurrency` / `useAccountingCurrency`. If relative times or list formatting ship, add settings-aware `Intl.RelativeTimeFormat` / `Intl.ListFormat` helpers in `@mms/shared` — same ban on raw `toLocale*`.
