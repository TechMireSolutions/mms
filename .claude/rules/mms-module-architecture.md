...
- **Soft Deletion (Work UX)**: Prefer soft-delete over hard `DELETE`. API/FE camelCase `deletedAt` / `deletedBy`; SQL `deleted_at` / `deleted_by` (typed columns preferred). Schema strip / indexes → data-layer.
  - **Reference FE**: Contacts / Students / Teachers pattern — page-level `viewingDeleted` + list `includeDeleted`, row restore, bulk restore, omit Add/messaging/destructive create in trash mode. Map `ModuleTrashToggle` / keyboard-hook `showDeleted` only at the control boundary. Same pattern shipped on Sessions, Attendance, Enrollments, Finance, Accounting, Obligations, Hasanat, Examinations, Question Bank (questions), Users (`tenant_users.deleted_at`; `status` remains invite/suspend).
...
| **Soft-delete** | `DELETE` + `POST :id/restore` (+ bulk when applicable); Work page-level `viewingDeleted` + list `includeDeleted` trash UI (map `ModuleTrashToggle` / shortcuts `showDeleted` at the control boundary only); detail-drawer archive chrome via `WarningCallout` + Restore (not ad-hoc markup); hide messaging actions when deleted; manifest `softDelete` metadata. Schema strip → `mms-data-layer.md`. |
