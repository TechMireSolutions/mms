      ...
      <motion.div variants={cardVariants} className="space-y-6">
        <div className="p-4 rounded-xl border border-border/60 bg-muted/30 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
          ...
          <div className="flex items-center gap-2 font-mono text-3xs text-muted-foreground">
            <span className="bg-card px-2 py-0.5 rounded border border-border/50">{t('platform.maintenance.dbConnsHealthy')}</span>
            <span className="bg-card px-2 py-0.5 rounded border border-border/50">{t('platform.maintenance.pm2Active')}</span>
          </div>
        </div>
