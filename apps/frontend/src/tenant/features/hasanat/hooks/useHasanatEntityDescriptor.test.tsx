...
import { useHasanatEntityDescriptor } from "./useHasanatEntityDescriptor";
...
function renderHookWrapper() {
  let hookResult!: EntityDescriptor<Distribution>;
  const container = document.createElement("div");
  document.body.appendChild(container);
  const root = createRoot(container);

  function TestComponent() {
    hookResult = useHasanatEntityDescriptor();
    return null;
  }
...
describe("useHasanatEntityDescriptor", () => {
  it("returns a descriptor with i18n-resolved labels for all fields", () => {
    ...
    expect(descriptor.entityType).toBe("hasanat");
    expect(descriptor.singularLabel).toBe("Distribution");
