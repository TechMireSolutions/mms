              ...
              <div className={`${WORK_SURFACE} space-y-4 p-4 md:col-span-2`}>
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <div className="flex flex-wrap items-center gap-2 overflow-x-auto max-w-full">
                    <FormSelect id="filterCategory" value={categoryFilter} onChange={setCategoryFilter} options={categorySelectOptions} />
                    <SearchBar placeholder={t('messaging.search.placeholder')} value={search} onChange={setSearch} className="max-w-xs" />
                  </div>
                </div>
                    ...
                    {filteredTemplates.map((template) => (
                      <article key={template.id} className={`${WORK_SURFACE_INNER} space-y-3 p-3`}>
                        ...
                        <div className="flex flex-wrap items-center gap-1">
                          ...
                          {canWrite && <Button variant="outline" size="icon" onClick={() => void duplicate(template)} className={`h-7 w-7 min-h-7 min-w-7 rounded-lg border-primary/30 ${SEMANTIC_BG.primary} ${SEMANTIC_TEXT.primary} hover:bg-primary/15 hover:border-primary/40 shadow-none`} title={t('messaging.duplicateTemplate')} aria-label={t('messaging.duplicateTemplate')}><Files className="h-3.5 w-3.5" /></Button>}
                          {canWrite && template.id.startsWith('custom_') ? (
                            <>
                              <Button variant="outline" size="icon" onClick={() => edit(template)} className={`h-7 w-7 min-h-7 min-w-7 rounded-lg border-info/30 ${SEMANTIC_BG.info} ${SEMANTIC_TEXT.info} hover:bg-info/15 hover:border-info/40 shadow-none`} title={t('common.edit')} aria-label={t('common.edit')}><Edit3 className="h-3.5 w-3.5" /></Button>
                              <Button variant="outline" size="icon" onClick={() => onDeleteRequest(template.id)} className={`h-7 w-7 min-h-7 min-w-7 rounded-lg border-destructive/30 ${SEMANTIC_BG.destructive} ${SEMANTIC_TEXT.destructive} hover:bg-destructive/15 hover:border-destructive/40 shadow-none`} title={t('common.delete')} aria-label={t('common.delete')}><Trash2 className="h-3.5 w-3.5" /></Button>
                            </>
                          ) : (
                        ...
                        {filteredTemplates.map((template) => (
                          <TableRow key={template.id} className="transition-colors hover:bg-muted/5">
                            ...
                            <TableCell className="px-4 py-3 text-center"><div className="flex items-center justify-center gap-1">
                              <Button variant="outline" size="icon" onClick={() => void copyBody(template.body)} className="h-7 w-7 min-h-7 min-w-7 rounded-lg border-muted-foreground/30 bg-muted-foreground/5 text-muted-foreground hover:text-foreground hover:bg-muted-foreground/15 hover:border-muted-foreground/40 shadow-none" title={t('messaging.copyTemplate')} aria-label={t('messaging.copyTemplate')}><Copy className="h-3.5 w-3.5" /></Button>
                              {canWrite && <Button variant="outline" size="icon" onClick={() => void duplicate(template)} className={`h-7 w-7 min-h-7 min-w-7 rounded-lg border-primary/30 ${SEMANTIC_BG.primary} ${SEMANTIC_TEXT.primary} hover:bg-primary/15 hover:border-primary/40 shadow-none`} title={t('messaging.duplicateTemplate')} aria-label={t('messaging.duplicateTemplate')}><Files className="h-3.5 w-3.5" /></Button>}
                              {canWrite && template.id.startsWith('custom_') ? <><Button variant="outline" size="icon" onClick={() => edit(template)} className={`h-7 w-7 min-h-7 min-w-7 rounded-lg border-info/30 ${SEMANTIC_BG.info} ${SEMANTIC_TEXT.info} hover:bg-info/15 hover:border-info/40 shadow-none`} title={t('common.edit')} aria-label={t('common.edit')}><Edit3 className="h-3.5 w-3.5" /></Button><Button variant="outline" size="icon" onClick={() => onDeleteRequest(template.id)} className={`h-7 w-7 min-h-7 min-w-7 rounded-lg border-destructive/30 ${SEMANTIC_BG.destructive} ${SEMANTIC_TEXT.destructive} hover:bg-destructive/15 hover:border-destructive/40 shadow-none`} title={t('common.delete')} aria-label={t('common.delete')}><Trash2 className="h-3.5 w-3.5" /></Button></> : <span className="rounded border border-border/30 bg-muted/65 px-1.5 py-0.5 font-mono text-xs italic uppercase text-muted-foreground/60">{t('messaging.tagSystem')}</span>}
                            </div></TableCell>
                          </TableRow>
                        ))}
