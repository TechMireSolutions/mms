...
/**
 * Upsert Students form tabs via typed `/api/custom-tabs`, then delete rows removed in Setup.
 */
export async function syncStudentsCustomTabs(formTabs: TabDefinition[]): Promise<void> {
  ...
  await apiJson("/api/custom-tabs/bulk", {
    method: "PUT",
    body: JSON.stringify({ moduleId: "students", tabs }),
  });

  const existing = await apiJson<{ tabs: CustomTabApiRow[] }>("/api/custom-tabs?moduleId=students");
