...
import { useObligationEntityDescriptor } from "./useObligationEntityDescriptor";
...
function renderHookWrapper() {
  let hookResult!: EntityDescriptor<ObligationCollection>;
  const container = document.createElement("div");
  document.body.appendChild(container);
  const root = createRoot(container);

  function TestComponent() {
    hookResult = useObligationEntityDescriptor();
    return null;
  }
...
describe("useObligationEntityDescriptor", () => {
  it("returns a descriptor with i18n-resolved labels for all fields", () => {
    ...
    expect(descriptor.entityType).toBe("obligations");
    expect(descriptor.singularLabel).toBe("Obligation Collection");
