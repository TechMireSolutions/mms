  ...
  return (
    <>
      ...
      <div>
        ...
        <p className="mb-2 flex items-center gap-1 text-[10px] italic text-muted-foreground/80"><Info className="h-3 w-3 flex-shrink-0 text-primary/70" />{t('messaging.fallbackHint')}</p>
        ...
        <div className="mt-1 flex items-center justify-between text-[10px] text-muted-foreground">
          ...
          <div className="flex items-center gap-2 font-mono">
            {isSms && <span className={`rounded px-1.5 py-0.5 text-[9px] font-bold uppercase ${smsStats.isUnicode ? 'border border-warning/30 bg-warning/15 text-warning' : 'bg-muted text-foreground'}`}>{smsStats.isUnicode ? t('messaging.encodingUnicode') : t('messaging.encodingGsm')} • {t('messaging.smsSegmentStats', { segments: smsStats.totalSegments, remaining: smsStats.remainingInSegment })}</span>}
            <span>{message.length} {t('contacts.whatsapp.chars')}</span>
          </div>
        </div>
        {isSms && smsStats.isUnicode && <p className="mt-1 flex items-center gap-1 text-[10px] font-medium text-warning"><AlertCircle className="h-3 w-3 flex-shrink-0" />{t('messaging.unicodeWarning')}</p>}
      </div>
      ...
      <AnimatePresence mode="wait">
        {previewText && recipient && (
          <motion.div key={`${previewIndex}-${channel}`} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.15 }} className="rounded-xl border border-border/80 bg-muted/20 p-3.5 backdrop-blur-xs">
            <h5 className="mb-2.5 flex items-center justify-between text-[11px] font-bold text-muted-foreground">
              ...
              <div className="flex items-center gap-2">
                {isBulk && <div className="flex items-center gap-1 text-[10px]"><Button type="button" variant="ghost" size="icon" className="h-5 w-5" disabled={previewIndex <= 0} onClick={() => onPreviewIndexChange(Math.max(0, previewIndex - 1))} aria-label={t('common.previous')}><ChevronLeft className="h-3 w-3" /></Button><span className="font-mono">{previewIndex + 1}/{eligibleRecipients.length}</span><Button type="button" variant="ghost" size="icon" className="h-5 w-5" disabled={previewIndex >= eligibleRecipients.length - 1} onClick={() => onPreviewIndexChange(Math.min(eligibleRecipients.length - 1, previewIndex + 1))} aria-label={t('common.next')}><ChevronRight className="h-3 w-3" /></Button></div>}
            ...
            {isEmail ? (
              <div className="space-y-2 rounded-xl border border-border bg-card p-3 text-xs shadow-xs">
                <div className="border-b border-border/50 pb-2 text-[11px] text-muted-foreground"><div><span className="me-1 font-semibold text-foreground">{t('messaging.to')}:</span>{recipient.name} &lt;{recipient.email}&gt;</div><div className="mt-0.5"><span className="me-1 font-semibold text-foreground">{t('messaging.subjectLabel')}:</span>{personalizeMessage(subject || t('messaging.defaultSubject'), recipient, personalizeOptions)}</div></div>
                <div className="whitespace-pre-wrap leading-relaxed text-foreground">{previewText}</div>
              </div>
            ) : isSms ? (
              <div className="max-w-[88%] space-y-1 rounded-2xl rounded-tl-none border border-info/20 bg-info/10 p-3 text-xs text-foreground shadow-xs"><div className="whitespace-pre-wrap leading-relaxed">{previewText}</div><div className="flex items-center justify-between border-t border-info/10 pt-1 font-mono text-[9px] opacity-70"><span>{recipient.phone}</span><span>{t('messaging.smsSegmentBadge', { count: String(smsStats.totalSegments) })}</span></div></div>
            ) : (
              <div className="max-w-[88%] space-y-1.5 rounded-2xl rounded-tl-none border border-success/20 bg-success/10 p-3 text-xs text-foreground shadow-xs"><div className="flex items-center justify-between border-b border-success/15 pb-1 text-[10px] font-semibold text-success"><span>{t('messaging.whatsappBroadcast')}</span><span>{recipient.phone}</span></div><div className="whitespace-pre-wrap leading-relaxed">{previewText}</div><div className="flex items-center justify-end gap-1 font-mono text-[9px] text-success"><span>{formatDateTime(new Date())}</span><CheckCheck className="h-3.5 w-3.5" /></div></div>
            )}
          </motion.div>
        )}
      </AnimatePresence>
