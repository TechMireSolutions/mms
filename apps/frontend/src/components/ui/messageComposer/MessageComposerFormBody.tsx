          ...
          <motion.div key={`${previewIndex}-${channel}`} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.15 }} className="rounded-xl border border-border/80 bg-muted/20 p-3.5 backdrop-blur-xs">
            <h5 className="mb-2.5 flex items-center justify-between text-xs font-bold text-muted-foreground">
              ...
              <div className="flex items-center gap-2">
                {isBulk && <div className="flex items-center gap-1 text-xs"><Button type="button" variant="ghost" size="icon" className="min-h-11 min-w-11" disabled={previewIndex <= 0} onClick={() => onPreviewIndexChange(Math.max(0, previewIndex - 1))} aria-label={t('common.previous')}><ChevronLeft className="h-3 w-3" /></Button><span className="font-mono">{previewIndex + 1}/{eligibleRecipients.length}</span><Button type="button" variant="ghost" size="icon" className="min-h-11 min-w-11" disabled={previewIndex >= eligibleRecipients.length - 1} onClick={() => onPreviewIndexChange(Math.min(eligibleRecipients.length - 1, previewIndex + 1))} aria-label={t('common.next')}><ChevronRight className="h-3 w-3" /></Button></div>}
