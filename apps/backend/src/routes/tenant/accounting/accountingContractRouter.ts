...
import { accountingContract } from '@mms/shared';
...
export const accountingContractRouter: FastifyPluginAsync = async (fastify) => {
  const router = s.router(accountingContract, {
    listAccounts: async ({ query, request }: ContractRouteArgs<typeof accountingContract['listAccounts']>): Promise<unknown> => {
      ...
      try {
        if (query?.page !== undefined) {
          const result = await withTenant(String(request.tenant?.id), () => accountingUseCases.loadAccountsPage(query as Parameters<typeof accountingUseCases.loadAccountsPage>[0]), { readOnly: true });
          return { status: 200 as const, body: result };
        }
        const accounts = await withTenant(String(request.tenant?.id), () => accountingUseCases.loadAccounts(query as Parameters<typeof accountingUseCases.loadAccounts>[0]), { readOnly: true });
    ...
    listEntries: async ({ query, request }: ContractRouteArgs<typeof accountingContract['listEntries']>): Promise<unknown> => {
      ...
      try {
        if (query?.page !== undefined) {
          const result = await withTenant(String(request.tenant?.id), () => accountingUseCases.loadEntriesPage(query as Parameters<typeof accountingUseCases.loadEntriesPage>[0]), { readOnly: true });
          return { status: 200 as const, body: result };
        }
        const entries = await withTenant(String(request.tenant?.id), () => accountingUseCases.loadEntries(query as Parameters<typeof accountingUseCases.loadEntries>[0]), { readOnly: true });
    ...
    listFiscalYears: async ({ query, request }: ContractRouteArgs<typeof accountingContract['listFiscalYears']>): Promise<unknown> => {
      ...
      try {
        if (query?.page !== undefined) {
          const result = await withTenant(String(request.tenant?.id), () => accountingUseCases.loadFiscalYearsPage(query as Parameters<typeof accountingUseCases.loadFiscalYearsPage>[0]), { readOnly: true });
          return { status: 200 as const, body: result };
        }
