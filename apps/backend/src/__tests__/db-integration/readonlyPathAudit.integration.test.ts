...
async function buildReadCases(): Promise<ReadCase[]> {
  ...
  return [
    ...
    { label: 'teachers.listTeachersByWorkspace', run: (t) => teacherRepo.listTeachersByWorkspace(t) },
    { label: 'teachers.findTeacherById', run: (t) => teacherRepo.findTeacherById(t, missingId) },
