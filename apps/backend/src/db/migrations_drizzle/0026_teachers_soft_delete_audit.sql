-- Teachers soft-delete audit columns + hot active partial index (Students 0011 parity).
ALTER TABLE "teachers" ADD COLUMN IF NOT EXISTS "deleted_by" text;--> statement-breakpoint
ALTER TABLE "teachers" ADD COLUMN IF NOT EXISTS "deletion_reason" text;--> statement-breakpoint
UPDATE "teachers"
SET
...
WHERE "custom_data" ?| ARRAY['deletedAt', 'deletedBy', 'deletionReason'];--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "teachers_workspace_active_idx" ON "teachers" USING btree ("workspace_subdomain") WHERE "deleted_at" IS NULL;--> statement-breakpoint
