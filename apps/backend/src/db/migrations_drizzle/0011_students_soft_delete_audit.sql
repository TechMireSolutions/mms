-- Students soft-delete audit columns + hot active partial index (Contacts parity).
ALTER TABLE "students" ADD COLUMN IF NOT EXISTS "deleted_by" text;--> statement-breakpoint
ALTER TABLE "students" ADD COLUMN IF NOT EXISTS "deletion_reason" text;--> statement-breakpoint
UPDATE "students"
SET
...
WHERE "custom_data" ?| ARRAY['deletedAt', 'deletedBy', 'deletionReason'];--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "students_workspace_active_idx" ON "students" USING btree ("workspace_subdomain") WHERE "deleted_at" IS NULL;--> statement-breakpoint
