...
function activeWorkspaceWhere(subdomain: string): SQL {
  return and(eq(financeInvoices.workspaceSubdomain, subdomain), isNull(financeInvoices.deletedAt))!;
}
...
function resolveSqlColumn(field: string): SQL {
  ...
  return sql.raw(`"finance_invoices"."${col}"`);
}
...
export async function aggregateFinanceWidgetQueries(
  tenant: string,
  queries: WidgetQuery[],
