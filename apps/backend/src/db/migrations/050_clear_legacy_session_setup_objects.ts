/**
 * Deletes orphan document-store Sessions Setup keys after typed-table backfill (049).
 * Safe to re-run: skips delete when typed Setup rows are missing for that tenant.
 */
import { eq } from 'drizzle-orm';
import { parseTenantScopedStorageKey, SESSIONS_MODULE_MANIFEST } from '@mms/shared';
import * as schema from '../schema.js';
import { deleteObjectByStorageKey, listObjectStorageKeys } from '../database.js';
import { withTenantTransaction } from '../withTenantTransaction.js';

const SETTINGS_KEY = SESSIONS_MODULE_MANIFEST.settingsObjectKey;
const COLUMN_PREFS_KEY = SESSIONS_MODULE_MANIFEST.columnPreferencesObjectKey;
...
export async function runMigration050(): Promise<void> {
  ...
  if (candidates.length === 0) {
    console.log('[Migration 050] No orphan Sessions Setup object keys to clear.');
    return;
  }
  ...
  await withTenantTransaction(null, async (tx) => {
    ...
    for (const candidate of candidates) {
      ...
      if (!allowed) {
        skipped += 1;
        console.warn(
          `[Migration 050] Skipping "${candidate.key}": typed Sessions Setup missing for tenant "${candidate.tenant}".`,
        );
        continue;
      }
  ...
  for (const key of toDelete) {
    ...
    console.log(`[Migration 050] Deleted orphan object key "${key}".`);
  }
  ...
  if (toDelete.length === 0 && skipped === 0) {
    ...
    console.log(
      `[Migration 050] Removed ${toDelete.length} key(s); skipped ${skipped} unsafe key(s).`,
    );
  }
}
