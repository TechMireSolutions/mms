...
import { parseTenantScopedStorageKey, splitTeachersSettingsBlob } from '@mms/shared';

const SETTINGS_KEY = 'teachers_settings';
const COLUMN_PREFS_KEY = 'teacher_user_column_preferences';

/** One-shot backfill: document-store teachers_settings / column prefs → typed Teachers Setup tables. */
export async function runMigration047(): Promise<void> {
  const db = getDb();
  ...
  await withTenant(null, async (tx) => {
    for (const [tenant, raw] of settingsByTenant) {
      const { fieldConfig, preferences } = splitTeachersSettingsBlob(raw);
      ...
      if (existingField.length === 0) {
        await tx.insert(schema.teacherFieldConfigs).values({
          workspaceSubdomain: tenant,
          config: fieldConfig,
          updatedAt: new Date(),
        });
        fieldCount += 1;
        console.log(`[Migration 047] Migrated teachers_settings field-config for tenant "${tenant}".`);
      }
      ...
      if (existingPrefs.length === 0) {
        await tx.insert(schema.teacherModulePreferences).values({
          workspaceSubdomain: tenant,
          preferences: preferences as unknown as Record<string, unknown>,
          updatedAt: new Date(),
        });
        prefsCount += 1;
        console.log(`[Migration 047] Migrated teachers_settings preferences for tenant "${tenant}".`);
      }
    }
    ...
    for (const [tenant, map] of columnByTenant) {
      ...
      for (const [userId, prefs] of Object.entries(map)) {
        ...
        await tx.insert(schema.teacherUserColumnPrefs).values({
          workspaceSubdomain: tenant,
          userId,
          preferences: prefs,
          updatedAt: new Date(),
      ...
      if (Object.keys(map).length > 0) {
        console.log(`[Migration 047] Migrated teacher column prefs for tenant "${tenant}".`);
      }
    }
  });

  console.log(
    `[Migration 047] Done — field-config=${fieldCount}, preferences=${prefsCount}, column-user-rows=${columnUserCount}.`,
  );
}
